        ...
        <section class="text-gray-600 px-4">
            <div class="flex flex-col h-full">
                <div class="w-full max-w-2xl mx-auto shadow rounded-md bg-white rounded-sm border border-gray-200">
                    ...
                    <!-- Search Bar -->
                    <div class="p-4">
                        <form on:submit|preventDefault={search} class="flex items-center">
                            ...
                            <div class="relative w-full">
                                ...
                                <input bind:value={searchQuery} type="text" id="simple-search"
                            ...
                            <div class="m-auto">
                                <div class="inline-block font-medium text-gray-800">
                                    {#if $page.params.page > 1}
                                        <a class="ui-button border-black float-left p-2 no-underline" rel="external"
                                           href={beforePage}>&laquo;</a>
                                    {:else}
                                    ...
                                    {#each Array.from({length: maxPage}) as i, pageNumber}
                                        ...
                                        {:else}
                                            <a class="border-black float-left p-2 no-underline" rel="external"
                                               href={'/articles/page/' + (pageNumber + 1) + hrefEndPaginator}>{(pageNumber + 1)}</a>
                                        {/if}
                                    {/each}

                                    {#if $page.params.page < maxPage}
                                        <a class="ui-button border-black float-left p-2 no-underline" rel="external"
                                           href={afterPage}>&raquo;</a>
                                    {:else}
